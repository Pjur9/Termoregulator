...
namespace Regulator
{
    public class RegulatorServer : IRegulator
    {
        ...
        {
            counter++;
            int br = BazaPodataka.GetDevices();
            temp.Add(avg);
           while (br != counter)
            {
                wc++;
        ...
        public bool poredjenje(double avg)
        {
                if (avg > 100 || avg < -10)
                {
        ...
        {
            ChannelFactory<IPec> factory = new ChannelFactory<IPec>(new NetTcpBinding(), new EndpointAddress("net.tcp://localhost:60003/IPec"));
